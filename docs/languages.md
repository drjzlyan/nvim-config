...
The IDE supports configurable per-language development environments. Users
choose which languages to set up during installation and can add or remove
languages at any time without affecting existing settings.
...
2. The script writes the selection to `~/.local/share/nvim/languages.local`
   (one language per line, `#` for comments).
3. `nvim-config/lua/languages/init.lua` reads that file at startup and
   conditionally loads the matching module under `lua/languages/`.
4. `scripts/install-tools.sh` reads the same file and installs only the
...
```bash
# Interactive menu (toggle languages by number)
...
- **Debugger**: `delve` (reserved for future DAP integration)
...
2. Add an entry to `ALL_LANGUAGES` in `dotfiles/scripts/languages.sh`:
   ```
   "name:Display Name — tools, that, are, installed"
   ```
...
5. Add tool versions to `scripts/tools.lock` if applicable.

6. Add install logic to `scripts/install-tools.sh` under a
   `if has_language "name"; then ... fi` block.

7. Add a check case to `check_language_tools()` in `dotfiles/doctor.sh`.
...
non-comment line is a language name that matches a module in
`lua/languages/`.
